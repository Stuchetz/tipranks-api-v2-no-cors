    ...
    .request(options, (resp) => {
      ...
      resp.on('end', () => {
        if ((resp.headers['content-type'] || '').includes('application/json')) {
          ...
          retry > 3
            ? reject()
            : setTimeout(get, 1000, retry, query, resolve, reject);
        }
      });
    })
    .on('error', (err) => {
